...
def gen_report(act, est_results):
    # 将act转换为列表（如果是numpy数组的话）
    act = act.tolist() if isinstance(act, np.ndarray) else act
    for key in est_results:
        value = est_results[key]
        if isinstance(value, np.ndarray):
            est_results[key] = value.tolist()
...
if __name__ == '__main__':
    ...
    train_json_file = 'data/new_train_data.json'
    test_json_file = 'data/you_can_make_it.json'
